...
var ImgIputHandler = {
	...
	rep : function(){
		...
		for ( var i = 0; i < $com.length; i++) {
			var str = $com[i].innerHTML;
			var facen = str.substring(str.indexOf("["), str.indexOf("]")+1);
			if(isNaN(facen)){
			var restr = function(){
				for ( var j = 0; j < ImgIputHandler.facePath.length; j++) {
					if (facen == "["+ImgIputHandler.facePath[j].faceName+"]") {
						return "<img title=\""
								+ ImgIputHandler.facePath[j].faceName
								+ "\" src=\"face/"
								+ ImgIputHandler.facePath[j].facePath
								+ "\" />";
					}
				}
			}
